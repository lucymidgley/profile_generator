...
const askquestion = function(qArray, n, aArray) {
  ...
  if (n === qArray.length) {
    rl.close();
    return console.log(`\n${aArray[0]} really enjoys to ${aArray[1]} on the weekends whilst listening to ${aArray[2]}. While ${aArray[0]} truly despises ${aArray[3]},  ${aArray[0]} loves to eat ${aArray[4]}. If ${aArray[0]} was stranded on a desert island and could only listen to one song, it would be ${aArray[5]}. In ${aArray[0]}'s world, no superpower is more desirable than the ability to ${aArray[6]}.`);
  } else {
